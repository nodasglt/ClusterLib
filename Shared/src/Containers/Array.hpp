...
class Array
{
  ...
  public:
    ...
    Array (const Array& other) : Array()
    {
        ...
        for (; mLength < other.getLength(); ++mLength)
        {
    ...
    template<typename U>
    Array (const Array<U>& other) : Array()
    {
        reserve(other.getCapacity());
    ...
    Array& remove (const sizeType index)
    {
        if (index > mLength) throw std::out_of_range( "Index out of range." );
    ...
    Array& remove (constIteratorType it)
    {
        return remove(sizeType(it - mData));
    }
    ...
    {
        if (mLength != other.mLength) return true;
